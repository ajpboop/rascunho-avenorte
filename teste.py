...
if "arranjos" not in st.session_state:
    st.session_state.arranjos = {
        # CP1 & CP3 juntos tinham 2x Inv 75 (1 para cada CP)
        "Arranjo 1 (CP1)": ArranjoFV(
            nome="Arranjo 1 (CP1)",
            potencia_pico_kwp=97.5,
            potencia_ca_kw=75.0,  # 1x 75 kW
        ),
        ...
        "Arranjo 4 (CP4)": ArranjoFV(
            ...
            potencia_ca_kw=400.0,  # 4x 100 kW
        ),
        "Arranjo 5 (CP5)": ArranjoFV(
            nome="Arranjo 5 (CP5)",
            potencia_pico_kwp=260.0,
            potencia_ca_kw=200.0,  # 2x 100 kW
        ),
...
# Relacionamento de Vinculação no State (Qual SE cada Arranjo está conectado)
if "vinculos" not in st.session_state:
    # Exemplo de inicialização: Arranjo -> SE
